...
ros2 run nav2_map_server map_saver_cli -f maps/<world>
  ...
  \item \textbf{[CRITICAL] Single end-to-end smoke test before batch.}
    Run one trial of \texttt{head\_on\_single} with the \texttt{full} controller,
    seed 0. Manually verify:
    \begin{itemize}
      \item All topics publishing: \texttt{/tracked\_people}, \texttt{/min\_human\_distance},
        \texttt{/collision\_event}, \texttt{/gt\_ttc}, \texttt{/cbf/intervention},
        \texttt{/cmd\_vel\_safe}, \texttt{/trial\_result}
      \item \texttt{metrics\_logger} writes a valid CSV with all 10 columns
      \item Pedestrian spawns and moves along its waypoints
      \item Nav2 sends the robot to the goal (or times out gracefully)
    \end{itemize}
    \textbf{Do not launch the batch until this passes.} One bad trial is cheap; 1,710
    bad trials is 85 hours of useless data.
